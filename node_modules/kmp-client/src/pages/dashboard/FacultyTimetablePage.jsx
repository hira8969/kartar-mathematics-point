...
export default function FacultyTimetablePage() {
  ...
  const exportPdf = () => {
    downloadTimetablePdf({
      fileName: "faculty-timetable.pdf",
      title: "Faculty Weekly Timetable",
      subtitle: "Published teaching schedule",
      entries: entries || [],
      branding
    });
  };

  if (!entries || !branding) return <LoadingState label="Loading faculty timetable..." />;

  return (
    <div className="space-y-6">
      <PageHeader eyebrow="Timetable" title="Faculty teaching timetable" description="Review the weekly timetable assigned by admin for your classes, batches, and teaching periods." />
      ...
      <SectionCard title="Weekly Schedule">
        <div className="space-y-5">
                ...
                {group.items.map((entry) => {
                  ...
                  return (
                    <div key={entry._id} className={`rounded-3xl border p-4 shadow-sm ring-2 ${theme.card} ${theme.dayRing}`}>
                      ...
                      <p className="mt-1 text-xs uppercase tracking-wide text-slate-400">Published for: {(entry.audienceRoles || []).join(", ")}</p>
          ...
          )) : <EmptyState title="No timetable assigned yet" description="Admin timetable entries for your teaching schedule will appear here." />}
        </div>
      </SectionCard>
